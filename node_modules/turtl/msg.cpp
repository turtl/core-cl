...
class lisp_data {
	public:
		...
		lisp_data(unsigned long msg_length, const char *msg)
		{
			size = msg_length;
 ...
 */
void TURTL_CONV main_message_handler(unsigned long msg_length, const char *msg)
{
...
void msg_node(uv_async_t *handle, int status)
{
	...
	printf("node: got lisp msg: %d\n", lispstr->size);

	TryCatch try_catch;

	// call our message callback
	Local<Value> argv[] = { String::New(lispstr->data, lispstr->size) };
...
NAN_METHOD(send_msg_lisp) {
	...
	turtl_msg_to_lisp((unsigned long)uistr.length(), uistr.c_str());
